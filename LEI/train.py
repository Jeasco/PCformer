...
parser.add_argument('--train-dataset', type=str, default='./datasets/ImageNet', help='dataset folder for train')
parser.add_argument('--save-epoch-freq', type=int, default=1 , help='how often to save model')
parser.add_argument('--print-freq', type=int, default=200, help='how often to print training information')
parser.add_argument('--train-visual-freq', type=int, default=200, help='how often to visualize training process')
...
def build_model(opt):
    ...
    psnrloss = PSNRLoss()
    loss = {'l1':l1loss,'ssim':ssimloss,'carloss':carloss,'edgeloss':edgeloss,'contrastloss':contrastloss,'psnrloss':psnrloss}

    return model, vgg, optimizer, loss, scheduler
...
def save_checkpoint(epoch, model, optimizer, iter=0):
    ...
    file_name = os.path.join(opt.output_dir, f'ckpt_epoch_{epoch}_{iter}.pth')
